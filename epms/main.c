...
Java_com_besl_nibss_epms_ISO8583_packEPMSISO8583Message(JNIEnv *env, jobject thisObj, jstring fields, jstring key)
{
    ...
    cjson = cJSON_GetObjectItem(cjsonObject, "56");
    if (cjson != NULL)
    {
        maxfld = atoi("56");
        (void)DL_ISO8583_MSG_SetField_Str(56, (const DL_UINT8 *)cjson->valuestring, &isoMsg); // Reason code
    }
    ...
    cjson = cJSON_GetObjectItem(cjsonObject, "63");
    if (cjson != NULL)
    {
        maxfld = atoi("63");
        (void)DL_ISO8583_MSG_SetField_Str(63, (const DL_UINT8 *)cjson->valuestring, &isoMsg); // PAN
    }
    ...
    for (j = 0; j < (isoHandler.fieldItems); j++)
    {
        ...
        if (NULL != isoMsg.field[j].ptr) // present
        {
            ...
            if (j == 90)
            {
                memset(buf, 0, sizeof(buf));
                memcpy(buf, isoMsg.field[j].ptr, isoMsg.field[j].len);
                cJSON_AddStringToObject(root, "90", buf);
            }
            ...
            if (j == 95)
            {
                memset(buf, 0, sizeof(buf));
                memcpy(buf, isoMsg.field[j].ptr, isoMsg.field[j].len);
                cJSON_AddStringToObject(root, "95", buf);
            }
